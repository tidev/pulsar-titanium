...
var Ti = {
    ...
    iosDevelopmentCertificates: function() {
        var certificates = [];
        if (Ti.info.ios && Ti.info.ios.certs) {
            for (let keychain in Ti.info.ios.certs.keychains) {
                certificates = certificates.concat(Ti.info.ios.certs.keychains[keychain].developer);
            }
        }
        return certificates;
    },

    iosDevelopmentProvisioningProfiles: function(certificate = {}) {
        var pem;
        if (certificate.pem) {
        ...
        if (Ti.info.ios && Ti.info.ios.provisioning) {
            // profiles = Ti.info.ios.provisioning.development;
            Ti.info.ios.provisioning.development.forEach(function(profile){
