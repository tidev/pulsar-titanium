...
const suggestions = {
			...
			didGenerateCallback: function (text, filePath) {
				return {
					...
					callback: async function () {
						const te = await atom.workspace.open(filePath, {
							searchAllPanes: true
						});
						let insertText = Utils.getConfigSetting('titanium.codeTemplates.tssClass');
			...
			didGenerateCallback: function (text, filePath) {
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.getConfigSetting('titanium.codeTemplates.tssId');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
			...
			didGenerateCallback: function (text, filePath) {
				...
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.getConfigSetting('titanium.codeTemplates.tssTag');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
		...
		{ // handler
			...
			didGenerateCallback: function (text) {
				...
				return {
					...
					showAlways: true,
					callback: async function () {
						const te = await atom.workspace.open(relatedPath, {
							searchAllPanes: true
						});

						let insertText = Utils.getConfigSetting('titanium.codeTemplates.jsFunction');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
	...
	common: [
		{ // i18n
			...
			targetPath: function () {
				return path.join(Utils.getI18nPath(), Utils.getConfigSetting('titanium.project.defaultI18nLanguage'), 'strings.xml');
			},
			didGenerateCallback: function (text) {
				const defaultLang = Utils.getConfigSetting('titanium.project.defaultI18nLanguage');
				...
				return {
					...
					callback: async function () {
						...
						if (!Utils.fileExists(i18nStringPath)) {
							mkdirSync(path.join(Utils.getI18nPath(), defaultLang));
							fs.writeFileSync(i18nStringPath, '<?xml version="1.0" encoding="UTF-8"?>\n<resources>\n</resources>');
						}
						const te = await atom.workspace.open(i18nStringPath, {
							searchAllPanes: true
						});
	...
	if (!textBuffer.isEmpty()) {
		textBuffer.scan(regExp, function (item) {
			...
			callback = {
				...
				callback: async function () {
					const te = await atom.workspace.open(path, {
						searchAllPanes: true
					});
