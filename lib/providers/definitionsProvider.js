...
const suggestions = {
			...
			didGenerateCallback: function (text, filePath) {
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.get('appcelerator-titanium.codeTemplates.tssClass');
			...
			didGenerateCallback: function (text, filePath) {
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.get('appcelerator-titanium.codeTemplates.tssId');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
			...
			didGenerateCallback: function (text, filePath) {
				...
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.get('appcelerator-titanium.codeTemplates.tssTag');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
			...
			didGenerateCallback: function (text) {
				...
				return {
					...
					callback: async function () {
						...
						let insertText = Utils.get('appcelerator-titanium.codeTemplates.jsFunction');
						insertText = insertText.replace(/(\${text})/g, text).replace(/\\n/g, '\n');
						const lastRow = te.getLastBufferRow();
						const lastPosition = [ lastRow, te.lineTextForBufferRow(lastRow).length ];
						te.setTextInBufferRange([ lastPosition, lastPosition ], insertText);
	...
	common: [
		{ // i18n
			...
			targetPath: function () {
				return path.join(Utils.getI18nPath(), Utils.get('appcelerator-titanium.project.defaultI18nLanguage'), 'strings.xml');
			},
			didGenerateCallback: function (text) {
				const defaultLang = Utils.get('appcelerator-titanium.project.defaultI18nLanguage');
