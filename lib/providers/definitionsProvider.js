...
var selectorFactory = {
	...
	lFunction: {
		...
		targetPath: function () {
			var i18nPath = Utils.getI18nPath();
		...
		didGenerateCallbacks({ callbacks, text }) {
			...
			var i18nStringPath = path.join(Utils.getI18nPath(), defaultLang, "strings.xml");
...
var regExps = {
	...
	js: [
		{ //require (/lib) name
			...
			targetPath: function (text, sourcePath) {
				var libFilePath = path.join(Utils.getAlloyRootPath(), 'lib', text + '.js');
				return libFilePath;
			}
		},
		{ //controller name
...
function findDefinition(path, regExp, textEditor, rightLabel) {
	...
	var targetEditor = Utils.getFileEditor(path);
...
export default {
	...
	getSuggestionForWord(textEditor, text, range) {
		...
		// console.log(`${regExps} ${fileExt} ${matchedExpItem}`);
