...
export default {
	/**
	 * Sort completions
	 * - by fuzzy score (if query is provided)
	 * - by priority
	 * - by text length
	 *
	...
	sort(suggestions, query) {
		if (query && typeof atom.ui !== 'undefined' && atom.ui.fuzzyMatcher && typeof atom.ui.fuzzyMatcher.score === 'function') {
			const searchTerm = query.replace(/[^a-zA-Z0-9]+$/, '');
			if (searchTerm) {
				for (const s of suggestions) {
					if (s.score === undefined) {
						const candidate = s.text || s.displayText || s.snippet || '';
						s.score = atom.ui.fuzzyMatcher.score(candidate, searchTerm);
					}
				}
			}
		}
