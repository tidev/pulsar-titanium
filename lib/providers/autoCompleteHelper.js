...
const completionsFormat = completion.CompletionsFormat.v3;

export default {
	/**
	 * Sort completions
	 * - by priority, so deprecated suggestions stay at the bottom
	 * - by fuzzy score (if a query is provided)
	 * - by text length
	 *
	 * autocomplete-plus re-scores and re-sorts anything from a provider with
	 * filterSuggestions enabled, so the scores below never reach the suggestion list.
	 * They still matter: it derives a sortScore from each suggestion's index first, so the
	 * order we hand back is used as a ranking prior. Scores are kept off the suggestion
	 * objects so they can't be confused with the score autocomplete-plus sets itself.
	 *
	 * @param {Array} suggestions 	suggestion objects
	 * @param {String} query 		user-typed prefix for fuzzy scoring
	 *
	 * @returns {Array}
	 */
	sort(suggestions, query) {
		const scores = new Map();
		// a bare '.' or similar tells us nothing, so leave everything unscored
		const searchTerm = query ? query.replace(/[^a-zA-Z0-9]+$/, '') : '';
		if (searchTerm && typeof atom.ui !== 'undefined' && atom.ui.fuzzyMatcher && typeof atom.ui.fuzzyMatcher.score === 'function') {
			for (const suggestion of suggestions) {
				const candidate = suggestion.text || suggestion.displayText || suggestion.snippet || '';
				scores.set(suggestion, atom.ui.fuzzyMatcher.score(candidate, searchTerm));
			}
		}
	...
	async loadCompletions(sdkVersion) {
		...
		this.completions = await completion.loadCompletions(sdkVersion, completionsFormat);
		return this.completions;
	},
};
