...
export default {
	...
	async generateAutoCompleteSuggestions() {
		const force = atom.config.get('appcelerator-titanium.general.generateAutoCompleteSuggestions');
		const selectedSdk = Appc.selectedSdk();
		try {
			// Generate the completions
			const [ alloy, sdk ] = await Promise.all([
				completion.generateAlloyCompletions(force),
				completion.generateSDKCompletions(force, selectedSdk.fullversion, selectedSdk.path)
			]);
			if (sdk || alloy) {
		...
		this.loadCompletions(selectedSdk);
	},

	async loadCompletions(sdkVersion) {

		if (!sdkVersion) {
			sdkVersion = await Appc.selectedSdk();
		}
		if (this.completions) {
			return this.completions;
		}
		this.completions = await completion.loadCompletions(sdkVersion.fullversion);
		return this.completions;
	},
};
