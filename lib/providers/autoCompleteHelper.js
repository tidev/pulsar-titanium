...
export default {
    ...
    //             stdout: output => {
    //                 const npmPath = (platform() === 'darwin') ? 'lib/node_modules' : 'node_modules';
    //                 resolve(path.join(output.trim(), npmPath, 'alloy'));
    //             },
    //             stderr: output => {
    //                 console.error(error);
    //                 reject(error);
    ...
    generateAutoCompleteSuggestions() {
        ...
        const appcPath = path.join(homedir(), '.appcelerator');
        const version = fs.readFileSync(path.join(appcPath, 'install/.version'), 'utf8');
        const alloyPath = path.join(appcPath, 'install', version, 'package/node_modules/alloy');
        console.log(alloyPath);

        // const alloyPath = '/usr/local/lib/node_modules/alloy';
        // this.getAlloyNpmPath().then(alloyPath => {
            // console.log('alloy path: ' + alloyPath);
            const alloyApi = JSON.parse(fs.readFileSync(path.join(alloyPath, 'docs/api.jsca'), 'utf8'));
            const titaniumAPIPath = path.join(Appc.selectedSdk().path, 'api.jsca');
            // console.log(titaniumAPIPath);
            ...
            const types = {};
            const props = {};
            api.types.forEach((type, idx) => {
                if (type.deprecated) return;

                let propertyNamesOfType = []
                type.properties.forEach((prop, idx) => {
                ...
                types[type.name.replace(/Titanium\./g, 'Ti.')] = {
                    ...
                    events: type.events.map(e => e.name)
                }
            });
            ...
            fs.writeFile(completionsFilename, 
                'module.exports = ' + JSON.stringify({
                    version: 1,
                    sdkVersion: `${Appc.selectedSdk().fullversion}`,
                ...
                function (err) {
                    if (err) {
                        ...
                        // console.log('JSON saved to ' + completionsFilename);
                        atom.notifications.addSuccess(`Appcelerator Titanium: Auto-complete suggestions generated for Titanium SDK ${Appc.selectedSdk().fullversion}`);
                    }
            });
        // });
    }
