		...
		getCompletions(request) {
			...
			if (regResult) {
				...
				if (!textBuffer.isEmpty()) {
					...
					if (curTagName && tags[curTagName]) {
						let { apiName } = tags[curTagName];
						let curTagObject = types[apiName];
			...
			if (regResult) {
				...
				if (regResult && regResult.length === 4) {
					...
					if (apiName.lastIndexOf('.') === apiName.length - 1) {
						...
						shortApiName = apiName.substr(0, shortApiName.length - 1);
					}
				}
				...
				const apiObj = types[shortApiName];
				if (apiObj) {
...
export default {
	...
	async loadCompletions() {
		...
		properties = completions.titanium.properties;
		types = completions.titanium.types;
		tags = completions.alloy.tags;
	},
