...
module.exports = {
  ...
  loadCompletions() {
    this.properties = {};
    return {pseudoSelectors: this.pseudoSelectors, properties: this.properties, tags: this.tags, types: this.types} = require('../../include/completions');
  },
  ...
  getPreviousPropertyName(bufferPosition, editor) {
    ...
    while (row >= 0) {
      const line = editor.lineTextForBufferRow(row);
      const propertyName = __guard__(propertyNameWithColonPattern.exec(line), x => x[1]);
      if (propertyName) { return propertyName; }
      row--;
    }
  },
  ...
  getPropertyValueCompletions(request) {
    ...
    if (!this.properties[property]) { return null; } //and @properties[parentPropertyName]
    ...
    if (this.isPropertyValuePrefix(prefix)) {
      for (value of Array.from(values)) {
        if (firstCharsEqual(value.replace(/\"/g,''), prefix.replace(/\"/g,''))) {
  ...
  getPropertyNameCompletions({bufferPosition, editor, scopeDescriptor, activatedManually}) {
    ...
    if ((this.properties[parentObjName] != null ? this.properties[parentObjName].type : undefined) && __guard__(this.types[this.properties[parentObjName] != null ? this.properties[parentObjName].type : undefined], x => x.properties.length)) {
  ...
  getTagSelectorPrefix(editor, bufferPosition) {
    const line = editor.getTextInRange([[bufferPosition.row, 0], bufferPosition]);
    return __guard__(tagSelectorPrefixPattern.exec(line), x => x[1]);
  },
      ...
      _.each(this.tags, function(value,key) { 
        if (firstCharsEqual(key, prefix)) {
            return completions.push(autoCompleteHelper.suggestion({
              type: 'tag',
              text: key,
              description: (that.types[value.apiName] != null ? that.types[value.apiName].description : undefined)
            }));
          }
      });
    }
