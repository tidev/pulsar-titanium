...
export default {
	...
	async getSuggestions(request) {
		...
		if (completions) {
			autoCompleteHelper.sort(completions, request.prefix);
		}
		return completions;
	},
	...
	getTagNameCompletions({ prefix, editor, bufferPosition }) {
		...
		for (let tag in this.tags) {
			if (!prefix || Utils.firstCharsEqual(tag, prefix)) {
				completions.push(
					isClosing
						? this.buildCloseTagCompletion(tag, this.tags[tag])
						: this.buildTagCompletion(tag, this.tags[tag])
				);
			}
		}
		return completions;
	...
	getAttributeNameCompletions({ editor, bufferPosition, prefix }) {
		...
		let apiName = tagName;
		if (this.tags[tagName] && this.tags[tagName].apiName) {
			apiName = this.tags[tagName].apiName;
		}
		let events = [];
		if (this.types[apiName]) {
			events = this.types[apiName].events;
		}
	...
	getAttributeValues(attribute) {
		attribute = this.properties[attribute];
		return (attribute ? attribute.values : undefined)  ? (attribute  ? attribute.values : undefined) : [];
	},

	getTagAttributes(tag) {
		const type = this.types[this.tags[tag] ? this.tags[tag].apiName : undefined];
		if (type) {
