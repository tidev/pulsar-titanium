...
export default {
	...
	getSuggestions(request) {
		console.log(`${request.scopeDescriptor} ${request.prefix}`);
		...
		if (!completions || completions.length === 0) {
			completions = alloyCompletionRules.i18n.getCompletions(request);
		}
	...
	getAttributeValueCompletions({ editor, bufferPosition, prefix }) {
		...
		if ((attribute === 'id') || (attribute === 'class')) {
			let fileName;
			let sourceEditor = Utils.getFileEditor(related.getTargetPath('tss', currentPath));
			if (!sourceEditor.isEmpty()) {
				values = this.tokenTextForSelector(sourceEditor, attribute);
				fileName = sourceEditor.getPath().split('/').pop();
	...
	getPreviousTag(editor, bufferPosition) {
		...
		while (row >= 0) {
			const matches = tagPattern.exec(editor.lineTextForBufferRow(row));
	...
	getTagAttributes(tag) {
		const type = this.completions.types[this.completions.tags[tag] != null ? this.completions.tags[tag].apiName : undefined];
		if (type) {
