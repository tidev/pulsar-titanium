...
export default {
	...
	getAttributeNameCompletions({ editor, bufferPosition, prefix }) {
		let completions = [];
		let tagName = this.getPreviousTag(editor, bufferPosition);
	...
	getAttributeValueCompletions({ editor, bufferPosition, prefix }) {
		...
		if ((attribute === 'id') || (attribute === 'class')) {
			let fileName;
			let sourceEditor = Utils.getFileEditor(related.getTargetPath('tss'));
			if (!sourceEditor.isEmpty()) {
				values = this.tokenTextForSelector(sourceEditor, attribute);
			...
			// app.tss file
			//
			sourceEditor = Utils.getFileEditor(path.join(Utils.getTiProjectRootPath(), 'app', 'styles', 'app.tss'));
			if (!sourceEditor.isEmpty()) {
				values = this.tokenTextForSelector(sourceEditor, attribute);
				fileName = _.last(sourceEditor.getPath().split('/'));
				for (value of Array.from(values)) {
				...
				if (Utils.isExistAsDirectory(controllerPath)) {
					...
					for (let file of Array.from(files)) {
						if (currentPath !== file) { // exclude current controller
							...
							// completions.push(autoCompleteHelper.requireSuggestion({ 
							//   text: value,
							//   prefix,
							//   replacementPrefix: additionalPrefix + prefix,
							// }));
	...
	getPreviousTag(editor, bufferPosition) {
		...
		while (row >= 0) {
			let tag = Utils.__guard__(tagPattern.exec(editor.lineTextForBufferRow(row)), x => x[1]);
			if (tag) { return tag; }
			row--;
		}
	},
	...
	getPreviousAttribute(editor, bufferPosition) {
		...
		return Utils.__guard__(attributePattern.exec(line), x => x[1]);
	},
	...
	getTagAttributes(tag) {
		return Utils.__guard__(this.completions.types[this.completions.tags[tag] != null ? this.completions.tags[tag].apiName : undefined], x => x.properties) != null ? Utils.__guard__(this.completions.types[this.completions.tags[tag] != null ? this.completions.tags[tag].apiName : undefined], x => x.properties) : [];
	},
