...
import tssCleanup from './tssCleanup';
...
export default {
			...
			if (Project.isTitaniumApp) {
				...
				this.projectDisposable.add(
					atom.commands.add('atom-workspace', {
						...
						'titanium:cleanup-tss': () => tssCleanup.cleanupActiveEditor(),
				...
				atom.contextMenu.add({
					'atom-workspace': [
						...
						{ label: 'Clean up TSS', command: 'titanium:cleanup-tss', shouldDisplay: () => this.checkShouldDisplayTssCleanup() }
					]
				});
			} else {
	...
	async runBuildCommand() {
		...
		if (Utils.getConfigSetting('general.displayBuildCommandInConsole')) {
	...
	takeScreenshot() {
		...
		const filename = path.join(Utils.getConfigSetting('titanium.general.screenshotPath'), 'screenshot_' + new Date().getTime() + '.png');
	...
	checkShouldDisplayTssCleanup() {
		const editor = atom.workspace.getActiveTextEditor();
		return Boolean(editor) && tssCleanup.isCleanableStyleFile(editor.getPath());
	},
