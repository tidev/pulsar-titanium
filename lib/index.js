...
export default {
	...
	activate(state) {
		...
		Project.onModified(() => {
			...
			var cliCmd = atom.config.get('appcelerator-titanium.general.useTi') ? 'ti' : 'appc';
	...
	async loadProject() {
		...
		if (Project.isTitaniumApp || Project.isTitaniumModule) {
			...
			atom.config.onDidChange('appcelerator-titanium.general.useTi', (e) => {
				if (e.newValue) {
	...
	async runBuildCommand() {
		...
		if (atom.config.get('appcelerator-titanium.general.displayBuildCommandInConsole')) {
			var cliCmd = atom.config.get('appcelerator-titanium.general.useTi') ? 'ti build' : 'appc run';
	...
	runCleanCommand() {
		...
		this.console.clear();
		this.console.show(true);
		this.console.write(atom.config.get('appcelerator-titanium.general.useTi') ? 'ti clean' : 'appc ti clean');
