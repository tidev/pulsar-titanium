...
export default {
		...
		general: {
			...
			properties: {
				...
				screenshotPath: {
					...
					description: 'Set the full path where the screeshots are stored.',
					order: 2
				},
			...
			if (Project.isTitaniumApp) {
				...
				this.projectDisposable.add(
					atom.commands.add('atom-workspace', {
						...
						'appc:takeScreenshot': () => this.takeScreenshot(),
	 ...
	 */
	takeScreenshot() {
		const filename = atom.config.get('appcelerator-titanium.general.screenshotPath') + "screenshot_" + new Date().getTime() + '.png';
		...
		if (os.platform() === 'darwin') {
			...
			exec('xcrun simctl io booted screenshot  ' + filename, (error, stdout, stderr) => {
				if (error) {
					atom.notifications.addError('Error saving iOS screenshot');
					return;
				}
				// done
		...
		if (Appc.info.android.devices.length > 0 || Appc.info.android.emulators.length > 0) {
			exec('adb shell /system/bin/screencap -p > ' + filename, (error, stdout, stderr) => {
				if (error) {
					atom.notifications.addError('Error saving Android screenshot');
					return;
				}
				// done
