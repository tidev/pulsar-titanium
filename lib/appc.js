...
const Appc = {
	...
	run(opts) {
		...
		var runCmd = atom.config.get('appcelerator-titanium.general.useTi') ? 'build' : 'run';
		var errorMsg = atom.config.get('appcelerator-titanium.general.useTi') ? 'ti build' : 'appc run';
	...
	new(opts) {
		...
		this.testAppcCommand(success => {
			if (!success) {
				var errorMsg = atom.config.get('appcelerator-titanium.general.useTi') ? 'ti' : 'appc';
				this.showAppcCommandError('Error executing \'' + errorMsg + ' new\' command');
				...
				proc.on('close', (code) => {
					...
					if (code === 0) {
						...
						var errorMsg = atom.config.get('appcelerator-titanium.general.useTi') ? 'ti' : 'appc';
						atom.notifications.addError('Error executing \'' + errorMsg + ' new\' command, the application was not created');
					}
				});
			}
		});
	...
	/**
	 * Run `appc login` command
	 *
	 * @param {Object} opts						arguments
	 * @param {String} opts.username   			appc name
	 ...
	 */
	clean(opts) {
		var runArgs = atom.config.get('appcelerator-titanium.general.useTi') ? [ 'clean' ] : [ 'ti', 'clean' ];
		var errorMsg = atom.config.get('appcelerator-titanium.general.useTi') ? [ 'clean' ] : [ '', 'appc ' ];
		...
		proc.onWillThrowError(error => {
			this.showAppcCommandError('Error executing \'' + errorMsg + 'ti clean\' command');
			error.handle();
			opts.exit && opts.exit();
		});
