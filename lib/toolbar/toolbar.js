...
export default class ToolBar {
    ...
    constructor () {
        ...
        this.runSelect.onChange(this.updateRunSelect.bind(this));
        ...
        this.platformSelect.onChange(e => {
            this.targetSelect.removeOptions();
            if (e.value === 'android') {
                this.populateAndroidTargets();
            } else if (e.value === 'ios') {
                this.populateiOSTargets();
                this.runSelect.updateOption('dist-adhoc', {disabled: false});
            } else if (e.value === 'windows') {
                this.populateWindowsTargets();
            }
            if (e.value == 'android' || e.value == 'windows') {
                this.runSelect.updateOption('dist-adhoc', {disabled: true});
                if (this.runSelect.selectedOption().value == 'dist-adhoc') {
                    this.runSelect.selectFirst();
        ...
        certificateTitle.classList.add('toolbar-title', 'icon', 'icon-organization');
        ...
        provisioningProfileTitle.classList.add('toolbar-title', 'icon', 'icon-file');
        ...
        this.keystoreSettings = document.createElement('div');
        this.keystoreSettings.classList.add('code-signing');

        const keystorePathTitle = document.createElement('div');
        keystorePathTitle.classList.add('toolbar-title');
        keystorePathTitle.appendChild(document.createTextNode('Keystore:'));
        this.keystoreSettings.appendChild(keystorePathTitle);

        this.keystorePathInput = document.createElement('input');
        this.keystorePathInput.classList.add('input-text', 'native-key-bindings', 'keystore-input', 'keystore-path-input');
        this.keystoreSettings.appendChild(this.keystorePathInput);

        this.keystorePathButton = this.addButton(this.keystoreSettings,{
            icon: 'file-directory',
            tooltip: 'Browse',
            flat: true,
            class: 'button-left',
            callback: (e) => {
                ...
                if (filePaths && filePaths[0].length > 0) {
                    this.keystorePathInput.value = filePaths[0];
                }
            }
        });

        const keystoreAliasTitle = document.createElement('div');
        keystoreAliasTitle.classList.add('toolbar-title');
        keystoreAliasTitle.appendChild(document.createTextNode('Keystore alias:'));
        this.keystoreSettings.appendChild(keystoreAliasTitle);

        this.keystoreAliasInput = document.createElement('input');
        this.keystoreAliasInput.classList.add('input-text', 'native-key-bindings', 'keystore-input');
        this.keystoreSettings.appendChild(this.keystoreAliasInput);

        const keystorePasswordTitle = document.createElement('div');
        keystorePasswordTitle.classList.add('toolbar-title');
        keystorePasswordTitle.appendChild(document.createTextNode('Keystore password:'));
        this.keystoreSettings.appendChild(keystorePasswordTitle);

        this.keystorePasswordInput = document.createElement('input');
        this.keystorePasswordInput.setAttribute('type', 'password');
        this.keystorePasswordInput.classList.add('input-text', 'native-key-bindings', 'keystore-input', 'keystore-password');
        this.keystoreSettings.appendChild(this.keystorePasswordInput);

        const privateKeyPasswordTitle = document.createElement('div');
        privateKeyPasswordTitle.classList.add('toolbar-title');
        privateKeyPasswordTitle.appendChild(document.createTextNode('Private key password:'));
        this.keystoreSettings.appendChild(privateKeyPasswordTitle);

        this.privateKeyPasswordInput = document.createElement('input');
        this.privateKeyPasswordInput.setAttribute('type', 'password');
        this.privateKeyPasswordInput.classList.add('input-text', 'native-key-bindings', 'keystore-input', 'keystore-password');
        this.keystoreSettings.appendChild(this.privateKeyPasswordInput);
    ...
    updateRunSelect() {
        const value = this.runSelect.selectedOption().value;
        if (value === 'run') {
            this.runButton.setIcon('playback-play');
        ...
        } else if (value === 'dist-adhoc') {
            this.runButton.setIcon('rocket');
            this.platformSelect.setEnabled(true);
            this.targetSelect.setEnabled(false);
            this.toggleCodeSigningButton.setEnabled(true);
            this.populateCodeSigning();
        } else if (value === 'dist-appstore') {
            this.runButton.setIcon('package');
            this.platformSelect.setEnabled(true);
            this.targetSelect.setEnabled(false);
            this.toggleCodeSigningButton.setEnabled(true);
            this.populateCodeSigning();
    ...
    enableCodeSigning() {
        ...
        if (target.platform.value == 'ios' && (target.type == 'device' || this.runSelect.selectedOption().value == 'dist-adhoc' || this.runSelect.selectedOption().value == 'dist-appstore')
            || target.platform.value == 'android' && this.runSelect.selectedOption().value == 'dist-appstore') {
            this.toggleCodeSigningButton.setEnabled(true);
        } else {
            this.toggleCodeSigningButton.setEnabled(false);
            this.hideCodeSigning();
    ...
    showCodeSigning() {
        ...
        if (this.selectedTarget().platform.value == 'ios') {
            this.element.appendChild(this.codeSigning);
            this.codeSigningElement = this.codeSigning;
        } else if (this.selectedTarget().platform.value == 'android') {
            this.element.appendChild(this.keystoreSettings);
            this.codeSigningElement = this.keystoreSettings;
        }
    ...
    hideCodeSigning() {
        if (this.showingCodeSigning) {
            ...
            this.element.removeChild(this.codeSigningElement);
    ...
    keystore() {
        return {
            path: this.keystorePathInput.value,
            password: this.keystorePasswordInput.value,
            alias: this.keystoreAliasInput.value,
            privateKeyPassword: this.privateKeyPasswordInput.value
        }
    }
    ...
    setRunState(inProgress) {
        ...
        if (inProgress) {
            this.runButton.setIcon('primitive-square');
